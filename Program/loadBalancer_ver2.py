...
class LoadBalancer:
    ...
    def close_client_with_countdown(self, client_conn):
        ...
        """
        try:
            client_conn.sendall(b"Server is down. Connection will close in 5 seconds.")
            for i in range(5, 0, -1):
                time.sleep(1)
                try:
                    client_conn.sendall(f"{i}...".encode())
                except socket.error:
                    break  # 클라이언트가 닫힌 경우 루프 종료
            client_conn.sendall(b"Connection closed.")
        except socket.error:
    ...
    def start(self, host='localhost', port=8080):
        ...
        while True:
            ...
            # 활성화된 서버만 선택
            active_servers = [addr for addr, status in self.server_status.items() if status]
            if not active_servers:
                print("No active servers available. Closing client connection.")
                client_conn.close()
                continue

            # 서버 선택 (랜덤 방식 또는 다른 알고리즘 사용 가능)
