...
class SnakeClient:
    def __init__(self, host='localhost', port=8080):
        ...
        self.messages = []  # 수신한 일반 메시지 저장 리스트 추가
        ...
        while self.running:
            try:
                ...
                if not data:  # 연결 종료 시
                    ...
                    break
                try:
                    message = pickle.loads(data)  # 데이터 디코딩
                    if "snakes" in message:
                        # 게임 상태 업데이트 🐍
                        self.update_game_state(message)
                    elif "message" in message:
                        # 일반 메시지 처리
                        print(message["message"])
                        self.messages.append(message["message"])  # 메시지를 리스트에 추가
    ...
    def update_game_state(self, state):
        server_score = state.get("scores", {}).get(self.client.fileno(), 0)  # 서버 점수 확인
...
def main():
    ...
    while running and client.running:
        ...
        # 수신한 메시지 화면에 표시
        if client.messages:
            message_text = FONT.render(client.messages[-1], True, (255, 0, 0))
            screen.blit(message_text, (10, 420))
